...
export const RH_PCTS = [0.1, 0.25, 0.5] as const

export function rhPhase(now = Date.now()) {
  if (now < RH_OPENS) return 'prep' as const
  if (now < RH_HOLD) return 'open' as const
...
export function rhCommit(board: number, pct = RH_EST_PCT) {
