...
export async function handleGate(req, res) {
  ...
  if (action === 'login' && req.method !== 'GET') {
    if (!verifyLogin(body.wallet, body.message, body.signature)) {
      res.status(401).json({ success: false, error: 'bad wallet signature' })
      return
    }
    sess = { wallet: body.wallet }
    addCookie(res, cookieStr(SESS, sign(sess), Math.floor(ACCESS_MS / 1000)))
    const remembered = walletMemory().get(body.wallet)
    if (remembered?.until > Date.now()) {
      ...
      res.status(200).json({
        success: true,
        data: { ...publicState(ticket, paid, ip, sess), credited: true },
      })
      return
    }
    const hit = await findPayment(ticket, null, body.wallet)
    if (hit) {
    ...
    res.status(200).json({
      success: true,
      data: { ...publicState(ticket, paid, ip, sess), connected: true },
    })
    return
  }
