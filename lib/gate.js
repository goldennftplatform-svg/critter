/** $5 desk pass — SOL / USDC / QUEST. Zero SaaS. Preview then blur. */

import { createHmac, createHash, randomBytes, timingSafeEqual } from 'crypto'

export const MIN_USD = 5
export const ACCESS_MS = 30 * 24 * 60 * 60 * 1000
export const PREVIEW_MIN_MS = 2 * 60 * 1000
export const PREVIEW_MAX_MS = 5 * 60 * 1000
...
const PAID = 'hq_paid'
const DEFAULT_TREASURY = '66W7hThTvatdN7mgMT5ZAaCbpuMb7Uskun4bAfteQM5F'
...
function previewWindow() {
  return PREVIEW_MIN_MS + Math.floor(Math.random() * (PREVIEW_MAX_MS - PREVIEW_MIN_MS + 1))
}
...
async function findPayment(ticket, pastedSig) {
  const payTo = ticket.treasury || treasury()
  const sigs = []
  if (pastedSig) sigs.push(pastedSig.trim())
  ...
  for (const sig of sigs) {
    ...
    if (!memo && sig !== pastedSig?.trim()) continue
    const got = received(tx, payTo)
    const cover = coversQuote(got, ticket)
    if (cover.ok) return { sig, ...cover, memo }
  }
  return null
}

function publicState(ticket, paid, ip) {
  const now = Date.now()
  ...
  const remainingMs = Math.max(0, (ticket?.previewUntil || 0) - now)
  return {
    gated: true,
    paid: false,
    remainingMs,
    previewMs: ticket ? ticket.previewUntil - ticket.t0 : 0,
    memo: ticket?.memo || null,
    ip: ipTag(ip),
    checkout: ticket
      ? quotePay(
          {
            sol: ticket.quotes.sol,
            usdc: 1,
            quest: ticket.quotes.quest,
            solAmt: ticket.quotes.solAmt,
            usdcAmt: ticket.quotes.usdcAmt,
            questAmt: ticket.quotes.questAmt,
            at: ticket.quotes.at,
            sources: ticket.quotes.sources,
          },
          ticket.treasury,
          ticket.memo,
        )
...
function bindIp(ip, ticket, paid) {
  ...
  if (prev) {
    ticket.t0 = prev.t0
    ticket.previewUntil = prev.previewUntil
    ticket.memo = prev.memo || ticket.memo
    ticket.quotes = prev.quotes || ticket.quotes
    ticket.treasury = prev.treasury || ticket.treasury
  }
  mem.set(key, {
    t0: ticket.t0,
...
/** Block cache/watch/sync once the free preview is over. Returns true if response was sent. */
export async function protect(req, res) {
  if (!gateEnabled()) return false
  cors(res)
  const { ip, paid, ticket } = await ensureSession(req, res)
  if (paid?.paid && paid.until > Date.now()) return false
  if (ticket?.previewUntil && ticket.previewUntil > Date.now()) return false
  res.status(402).json({
    success: false,
    error: 'subscribe',
    remainingMs: 0,
...
export async function handleGate(req, res) {
  ...
  if (!gateEnabled()) {
    res.status(200).json({ success: true, data: { gated: false, paid: true, remainingMs: 0 } })
    return
  }

  const q = req.query || {}
  const body = readBody(req)
  let { ip, paid, ticket } = await ensureSession(req, res)
  ...
  if ((action === 'confirm' || body.sig) && req.method !== 'GET') {
    const hit = await findPayment(ticket, body.sig || q.sig)
    if (!hit) {
      res.status(200).json({
        success: true,
        data: { ...publicState(ticket, paid, ip), waiting: true },
      })
      return
    }
    paid = {
    ...
    res.status(200).json({
      success: true,
      data: { ...publicState(ticket, paid, ip), credited: true, sig: hit.sig, asset: hit.asset },
    })
    return
  }

  res.status(200).json({ success: true, data: publicState(ticket, paid, ip) })
}
