...
export function buildNextSteps(snap) {
  ...
  const phase = rhPhase()
  const board = Math.round(snap.boardQuest || 0)
  if (phase === 'prep' && board > 0) {
    push(
      'high',
      'RH · bind QUEST',
      `${board.toLocaleString()} on the board. Aug 20 takes a fixed % — all or nothing. Top-up is not opt-in.`,
    )
  } else if (phase === 'open' && snap.masterCount > 0) {
    push('critical', 'RH · OPT IN', 'Window is short. Binding more now does not enroll you.')
  }

  const blankBps = (snap.masters || []).filter((m) => m.bpsKnown === false && m.tokens > 0).length
  if (blankBps) {
    push(
      'mid',
      'BPS metadata blank',
      `${blankBps} masters lost BPS on the migrate JSON. Board QUEST still counts for RH.`,
    )
  }
