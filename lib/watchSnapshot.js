...
const NFT_JSON = 'https://nfts.critters.quest/json'
...
export function watchEntryFor(address, mine = false) {
  const wallet = String(address).trim()
  return {
    id: `${mine ? 'mine' : 'add'}-${wallet.slice(0, 8)}`,
    label: mine ? 'Your wallet' : 'Tracked wallet',
    tagline: mine ? 'Connected to this desk' : 'Added on this browser',
    wallet,
  }
}

export function mergeWatchlist(extraAddrs = []) {
  const extras = []
  const seen = new Set(WATCHLIST.map((w) => w.wallet))
  for (const [i, raw] of extraAddrs.entries()) {
    const wallet = String(raw || '').trim()
    if (!isSolanaAddress(wallet) || seen.has(wallet)) continue
    seen.add(wallet)
    extras.push(watchEntryFor(wallet, i === 0))
...
function questFromAccounts(list = []) {
  let n = 0
  for (const a of list) {
    const info = a.account?.data?.parsed?.info
    if (!info || info.mint !== QUEST_MINT) continue
    n += Number(info.tokenAmount?.uiAmount || 0)
  }
  return n
}

function boundQuest(list) {
  for (const key of ['Tokens', 'Bound', 'QUEST', '$QUEST', 'Locked']) {
    const v = getAttr(list, key)
    if (v == null || v === '' || /^none$/i.test(String(v))) continue
    const n = Number(v)
...
function masterNum(name) {
  return String(name || '').match(/#(\d+)/)?.[1] || null
}

async function fillMissingBps(names, bpsMap) {
  const need = names.filter((n) => n && !bpsMap.has(n))
  await Promise.all(
    need.map(async (name) => {
      const id = masterNum(name)
      if (!id) return
      try {
        const json = await fetch(`${NFT_JSON}/${id}.json`, { cache: 'no-store' }).then((r) => r.json())
        const bps = Number(
          (json.attributes || []).find((a) => String(a.trait_type).toUpperCase() === 'BPS')?.value || 0,
        )
        if (bps) bpsMap.set(name, bps)
      } catch {
        /* CDN miss after migrate */
      }
    }),
  )
}
...
export async function snapshotWallet(entry, bpsQuote) {
  ...
  const [mastersJson, balJson, tokJson, tok22Json, dasJson] = await Promise.all([
  ...
  await fillMissingBps(
    (mastersJson.assets || []).map((a) => a.name),
    bpsMap,
  )

  const liquidQuest =
    questFromAccounts(tokJson.result?.value) + questFromAccounts(tok22Json.result?.value)

  const masters = (mastersJson.assets || []).map((a) => {
    const list = a.attributes?.attributeList || []
    const name = a.name
    const tokens = boundQuest(list)
