...
export function buildNextSteps(snap) {
  ...
  if (snap.blindSpots) {
    push(
      'mid',
      'Open spectate map',
      'No account needed — watch the live Valdara map, then match what you see to next steps here.',
    )
  }
...
export async function snapshotWatchlist(list = WATCHLIST) {
  ...
  return {
    ...
    note: 'Roster is on-chain. Live map is free spectate (no login). Bank internals still need his session.',
    wallets,
  }
}
